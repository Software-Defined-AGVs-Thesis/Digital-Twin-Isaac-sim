...
######### we are here 
import threading
_vr_pause_event = threading.Event()
_vr_pause_event.set()  # Initially not paused
##########
...
####3 we are here again
def _on_vr_state_changed(event):
    """Pause simulation loop when VR is starting to avoid render context conflict."""
    import carb
    try:
        is_active = carb.settings.get_settings().get("/exts/omni.kit.xr.profile.vr/isActive")
        if is_active:
            _vr_pause_event.clear()  # Pause
        else:
            _vr_pause_event.set()    # Resume
    except:
        _vr_pause_event.set()
...
#     def register(self, name: str, lab_manager: Union[ROS_LunalabManager, ROS_LunaryardManager]) -> None:
#         self._lab_managers[name] = lab_manager

#     def __call__(self, cfg: dict, **kwargs) -> Union[ROS_LunalabManager, ROS_LunaryardManager]:
#         return self._lab_managers[cfg["environment"]["name"]](
#             environment_cfg=cfg["environment"],
#             **kwargs,
#         )
...
#     def __init__(self, cfg: dict, simulation_app: SimulationApp) -> None:
#         self.cfg = cfg
#         self.simulation_app = simulation_app
#         self.timeline = omni.timeline.get_timeline_interface()
#         self.world = World(
...
#         def _watch_vr_active(value, *args):
#             if value and not self._vr_active:
#                 self._vr_active = True
#                 print("[OmniLRS] VR starting — will pause after current frame")

#         import carb.settings
#         self._vr_watcher = carb.settings.get_settings().subscribe_to_node_change_events(
#             "/exts/omni.kit.xr.profile.vr/isActive",
#             _watch_vr_active
#         )
#         print("[OmniLRS] VR carb watcher registered")
...
#                             self.ROSLabManager.LC.deform_terrain()
#             else:
#                 # VR is active — skip world.step() entirely, just pump the app
#                 pass
